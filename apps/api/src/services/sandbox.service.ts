...
export class SandboxService {
  ...
  private static convertPythonToExecutableJs(pyCode: string): string {
    ...
    for (let i = 0; i < rawLines.length; i++) {
      ...
      // Handle indentation decrease -> pop open braces
      ...
      // print(...) -> console.log(...)
      line = line.replace(/\bprint\s*\(([\s\S]*?)\)$/, (_m, args) => {
      ...
      // return tuple / values: return a, b
      ...
      if (tupleAssignMatch && tupleAssignMatch[1].includes(',')) {
        const vars = tupleAssignMatch[1].split(',').map((s) => s.trim()).filter(Boolean);
